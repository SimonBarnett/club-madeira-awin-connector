# Functional specification — Club Madeira Awin connector

**Source:** [functional-spec-awin-connector-locked-2026-09-18.pdf](./functional-spec-awin-connector-locked-2026-09-18.pdf)  
**Date:** 2026-09-18 · **Publisher:** 2889699 Club Madeira  
**Audience:** build agent · Treat **LOCKED** rows as constants. Do not re-litigate.
...
| Join UX | Advertisers → Join Programmes → + Join. Modal: promotional type, optional message, T&Cs, Join |
| Message cap | **150** characters (official EN+FR help). Not 170 |
| After join | Pending. Track My Programmes → Pending or `GET programmes?relationship=pending` |
...
| Selection (brief) | RDS: ProductFeed=Yes, Joined=0, ApprovalRate≥99, exclude Mode=global 90 days, LastSeen DESC |
...
- Nav: `ui.awin.com` → Advertisers → Join Programmes (or profile Join Programme)
- Quick: **+ Join**
...
## 3. Remaining unknowns — only these eight

| ID | Unknown | Config / note |
|---|---|---|
| U1 | Promotion type — exact option in live modal | `AWIN_PROMOTION_TYPE` |
...
| U6 | Worker host name — always-on desktop with overnight Awin Chrome profile | |
| U7 | AM letter — private apply API yes/no + URL. Until written yes, B1 closed | |
| U8 | John email cutover — default: keep until T-UI-05 passes | |

U3–U5 not closable from public GitHub search of SimonBarnett (`clubscan` / `AwinHighApprovalMerchants` / `getAwinConfig` not visible).
...
## 6. First ticket — implement with no more questions

- TS client: GET `publishers` / `programmes` / `programmedetails`; POST `promotions`; 18/min; redaction.
- `criteria.ts` — 99 / ProductFeed / 90d / 150-char. RDS optional.
...
- README with locked list, Phase 0 curl, AM email, U1–U8 checklist.
