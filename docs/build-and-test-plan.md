# Build and test plan — Club Madeira Awin connector
...
### Goals (first shippable slice = “First ticket” in the spec)

1. TypeScript Awin **read** client for publisher `2889699`: publishers, programmes, programmedetails, promotions (POST read-only offers).
...
7. README documents locked constants, Phase 0 curls, AM email template, U1–U8 checklist.
...
| Method | Path | Notes |
|---|---|---|
| GET | `/publishers/2889699` | Smoke / Phase 0 |
| GET | `/publishers/2889699/programmes?relationship=` | `joined\|pending\|suspended\|rejected\|notjoined\|any` |
| GET | `/publishers/2889699/programmedetails?advertiserId=&relationship=` | KPI includes `approvalPercentage` |
| POST | `/publisher/2889699/promotions` | Offers; membership can include not-joined; **read-only** |
...
### Selection (when RDS available — U5)
...
Template from U3 when available; enforce `length ≤ 150`. Reject longer before queue.

### Queue / apply

- Persist candidates + state: `candidate | dry_run_queued | dry_run_done | blocked_needs_ui | applied_pending` (applied only after T-UI-05 — not in first ticket).
- `AWIN_JOIN_DRY_RUN=true` (default): apply records intent, **does not** open UI or call any fictional join API.
- Caps: ≤20 applies/day; ≥30s between submits; interactive batch 6–12; cron batch 12.

### CLI surface

- `list_join_candidates` — prints ranked list (redacted).
- `relationship_status` — pending/joined/notjoined counts or per-id.
...
```ts
// worker/stub.ts
export async function runJoinWorker(/* … */): Promise<{ status: 'needs_auth' }> {
...
| T-RATE-02 | 19th within window | waits or rejects per documented policy (prefer wait ≤2s in test with fake clock) |

### T-UI-05 — gated (NOT in first PR)
...
2. Headed screenshot of Join modal: type control, URL field yes/no, T&Cs — **Cancel**, do not Join.
3. Fill U1, U2; note U6 host name.
4. Optional AM email for U7 (template in functional-spec §5).
...
- [ ] P1–P4 complete; all T-SEL T-API T-APP T-RED T-RATE green in CI.
- [ ] Default dry-run is on; no code path submits a real Join.
- [ ] README lists locked constants, Phase 0, AM email, U1–U8.
...
Implement First ticket (P0–P4) exactly. LOCKED constants are not negotiable.
